...
| 2026-09-25 | GitHub Copilot | Paket G-5 — Penyeragaman Dokumentasi Kode Frontend | Gaya dokumentasi diseragamkan: **satu keterangan pendek per kelas/fungsi**, blok `@param` / `@return` / `@var` yang hanya mengulang type hint dan narasi historis dihapus. Idiosinkrasi kode ini adalah docblock 3 baris (`/**` + satu kalimat + `*/`) sebagai bentuk "satu keterangan", sehingga pemangkasan difokuskan pada blok **> 3 baris**; dari 116 docblock pada 35 file di `app/`, 22 dipangkas dan kini tidak ada lagi blok > 3 baris kecuali 1 pengecualian yang disengaja (`UserManagementService::deleteUser` — 2 baris karena memuat alasan teknis ON DELETE CASCADE). **Controllers**: `ScraperMonitorController::status`. **Services**: `AnalysisImportService::importPostsFromCsv`, `AnalysisService::{syncRunningAnalyses,streamExportCsv,sanitizeCsvCell}`, `FastAPIClientService::{sanitizeSessionData,classifySingle,getPipelineStatus,downloadExportToStream}` (blok 10 baris → 1 kalimat), `ProfileService::{updateProfileInfo,updatePassword}`, `SinglePredictService` (docblock kelas), `UserManagementService::{getFilteredUsers,getUserMetrics,getAssignableRoles,deleteUser}`. **Requests**: `FilterAnalysisRequest::rules`, `FilterPostRequest` (kelas + `rules`), `StoreAnalysisRequest::{prepareForValidation,rules}`, `SinglePredictRequest::rules`, `TriggerLoginRequest::{validationData,rules,messages}`, `FilterUserRequest` (kelas). **Models/Policies** (konsistensi): docblock kelas `Analysis`, `AnalysisClassification`, `AnalysisExport`, `AnalysisPost`, `AnalysisStatistic`, `SinglePrediction`, `AnalysisPolicy` (14 → 1 baris) dipangkas; tiga docblock berbahasa Inggris bawaan scaffold Laravel di `User.php` diterjemahkan ke satu baris Bahasa Indonesia. **Verifikasi**: `php -l` bersih pada 21 file · `php artisan test` **41 test / 102 assertion LULUS**. Tidak ada perubahan perilaku — murni komentar. |
